...
(defmodule MAIN
  (export deftemplate initial-fact
	  ...
	  too-close restriction restriction-event-obs
...
(defmodule OBS_CONSTRAINTS (import MAIN deftemplate observation time-overlapping same-module-time-overlapping 
        too-close restriction-obs-obs restriction-event-obs incompat-observations-time-overlapping)
...
(defrule OBS_CONSTRAINTS::too-close-rule
        (restriction-obs-obs (name too-close) (incompat-observations ?o1 ?o2) (delta ?d)) 
...
(defrule OBS_CONSTRAINTS::incompat-observations-time-overlap-rule
	...
	(restriction-obs-obs (name cant-overlap-observations) (incompat-observations ?n1 ?n2))
	=>
...
(defrule EVENT_CONSTRAINTS::observations-out-of-event-window
  ...
  (not (event (eventname ?ev1) (instance-id ?id2) (start-time &st2) (end-time &et2))
  (restriction-event-obs (name obs-in-event-window) (event ?ev1) (observation ?o1)) 
  =>
  (assert (observation-out-of-window (observation-1 ?o1) (instance-obs ?id1)
				     (event-1 ?ev1) (instance-event ?id2))))
